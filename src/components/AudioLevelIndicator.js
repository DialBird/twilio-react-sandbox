...
export const initializeAnalyser = (audioTrack) => {
...
const AudioLevelIndicator = ({ size, audioTrack, background }) => {
  const SIZE = size || 24
  const ref = useRef()
  ...
  useEffect(() => {
    ...
    if (audioTrack && isTrackEnabled && SVGClipElement) {
      ...
      const timer = interval(() => {
        ...
        if (SVGClipElement) SVGClipElement.setAttribute('y', String(21 - volume))
      }, 50)
